...
class OptionPremium extends WpSolrExtensions {
	...
	public function wpsolr_filter_include_file( $help_id ) {
		...
		switch ( $help_id ) {
			...
			case WPSOLR_Help::HELP_FACET_HIERARCHY:
				$file_name = 'search-facet-hierarchy.inc.php';
				break;

			case WPSOLR_Help::HELP_FACET_POST_TYPE:
				$file_name = 'search-facet-post-type.inc.php';
				break;
		...
		return ! empty( $file_name ) ? sprintf( '%s/includes/%s', dirname( __FILE__ ), $file_name ) : '';
	}
}